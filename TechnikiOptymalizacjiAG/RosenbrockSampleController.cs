...
{
    [DisplayName("Rosenbrock")]
    public class RosenbrockSampleController : MySampleControllerBase
    {
        ...
        public override IFitness CreateFitness()
        {
            return new FuncFitness((chromosome) =>
            {
                ...
                // Rosenbrocka
                //f(x)=sum_{i=0}^{n-1} [100(x_{i+1} - x_i^2)^2+(1-x_i^2 )^2]
                // TODO: Co z (x2,y2) ???
                return 100 * Math.Pow(y1 - Math.Pow(x1, 2), 2) + Math.Pow(1 - Math.Pow(x1, 2), 2); 
            });
        }
